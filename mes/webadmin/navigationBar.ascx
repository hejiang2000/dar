...
void Page_Load() {
    if(!Page.IsPostBack) {
        ...
        // initialize WebAppList here
        System.Collections.Specialized.NameValueCollection coll = ConfigurationManager.AppSettings;
        string defaultApp = coll["default"] ?? coll[0];
        if (Session != null) {
            if (Session[APP_PATH] == null || Session[APP_PHYSICAL_PATH] == null) {
                string[] paths = defaultApp.Split(';');
                Session[APP_PATH] = paths[0];
                Session[APP_PHYSICAL_PATH] = paths[1];
            } else {
                defaultApp = Session[APP_PATH] + ";" + Session[APP_PHYSICAL_PATH];
            }
        }
        foreach (string key in coll.AllKeys)
        {
            try {
                ListItem item = new ListItem(key, coll[key]);
                if (coll[key] == defaultApp) {
                    item.Selected = true;
                }
                WebAppList.Items.Add(item);
            } catch (Exception) {}
        }
...
protected void WebAppList_OnSelectedIndexChanged(Object sender, EventArgs e) {
    if (Session != null) {
        Session.Clear();
        string[] paths = WebAppList.SelectedItem.Value.Split(';');
        Session[APP_PATH] = paths[0];
        Session[APP_PHYSICAL_PATH] = paths[1];
...
//-->
            ...
            <table width="100%" height="64" cellspacing="0" cellpadding="0" class="homePageHeader" border="0">
                ...
                <tr>
                    <td colspan="3" valign="bottom" nowrap height="33">
                        ...
                        <table id="TabTable" width="100%" cellspacing="0" cellpadding="0" border="0" onselectstart="event.returnValue=false;">
                            <tr>
                                ...
                                <td width="81em" height="20" align="center" valign="baseline" nowrap class="<%#GetCenterClass(Container.ItemIndex)%>" 
                                ...
                                <td  align="right" height="25" valign="top" nowrap class="spacerTab">
                                <asp:DropDownList ID="WebAppList" runat="server" AutoPostBack="true" OnSelectedIndexChanged="WebAppList_OnSelectedIndexChanged">
                                </asp:DropDownList>&nbsp;</td>
                            </tr>
                        </table>
